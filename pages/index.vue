<template>
    <div style="height: 450dvh;" id="app_container">
      <div style="position: fixed; top: 0; right: 0; width: 100vw; height: 100dvh; overflow: hidden">
        <section class="presentation">
          <img
            fetchpriority="high"
            src="../assets/photoCV.webp"
            class="photo"
            alt="photo cv"
            type="image/webp"
          />
          <div class="text">
            ...
            <h2 style="margin: 20px; font-weight: 500; font-size: 18px">
              {{
                frSelect
        ...
        <section id="triangle-top-right" class="triangle-top-right">
          <DegreesCard />
        </section>
        <section id="triangle-top-left" class="triangle-top-left">
          <SkillsCard />
        </section>
        <section id="left-border" class="left-border">
          <PorteFolio />
        </section>
      </div>
    </div>
...
const innerHeight = computed(()=>window.innerHeight || 0)
...
watch(
  ...
  (y) => {
    const y_position = (y * 100) / innerHeight.value
    if (y_position < 1 && lastScrollPosition.value && lastScrollPosition.value < 1 && innerWidth.value>800)
      window.scrollTo({ top: (350 * innerHeight.value) / 100 })
    if (y_position < 10) {
    ...
    if (y_position < 220) {
      ...
      document.getElementById('portefolio-title').style.opacity = 0
      lastScrollPosition.value = y_position
      return
    }
    if (y_position < 320) {
      ...
      document.getElementById('portefolio-title').style.opacity = 1
      const bars = document.getElementsByClassName('bars')
      for (let i = 0; i < bars.length; i++) {
        bars[i].style.backgroundColor = 'var(--primary)'
      }
      ...
      if (innerWidth.value >= 800){
        document.getElementById('portefolio-right-section').style.transform =
          `translateY(${30 - translateY}dvh)`
        document.getElementById('portefolio-left-section').style.transform =
          `translateY(${-30 + translateY}dvh)`
      }
    ...
    if (y_position < 350) {
      ...
      lastScrollPosition.value = y_position
      return
    } else if (lastScrollPosition.value && lastScrollPosition.value > 10 && innerWidth.value>800)
