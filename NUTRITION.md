# Module nutrition — intégration

Onglet « Nutrition » ajouté à `/sport`. Il réutilise le profil (taille, sexe, année de
naissance) et le poids de corps déjà saisis dans le suivi de séances : une seule source
de vérité, aucun doublon de données.

## Fichiers ajoutés

| Fichier | Rôle |
|---|---|
| `data/nutritionProgram.ts` | Table des aliments (macros + micros), 16 recettes, cycle de 14 jours, sessions de batch cooking, compléments, références ANSES. **Généré** — voir « Régénérer les données ». |
| `lib/nutritionStats.ts` | Toute la logique pure : macros, construction d'une journée, Mifflin-St Jeor, liste de courses, budget, couverture en micronutriments. Aucun DOM, aucun localStorage. |
| `composables/useNutrition.ts` | État persisté (même pattern que `useWorkout` : refs module-scope + hydratation gardée par un flag). |
| `components/nutrition/Panel.vue` | Coquille + sous-navigation, deux onglets : Préparer (sélection → courses → préparation), Plats (avec les micronutriments). Le planning vit dans l'onglet Journal. |
| `components/nutrition/EatSheet.vue` | Feuille « remplir ma journée », ouverte depuis l'accueil. |
| `components/nutrition/Day.vue` | Contenu de cette feuille : repas, horaires, grammages, macros, **cible recalculée d'après la séance réellement enregistrée**, et consigne d'ajustement du dîner. |
| `components/nutrition/Hero.vue` | Bandeau d'accueil de `/sport` : camembert des calories, frise horaire de la journée, saisie en trois secondes. |
| `components/sport/DaySheet.vue` | Tout ce qu'il y a à savoir sur UNE journée : séance, repas, cible calorique, pesée, pas, réglages salle/télétravail. S'ouvre au clic sur une case du calendrier. |
| `components/nutrition/Library.vue` | Bibliothèque de plats et d'aliments, avec éditeurs. |
| `components/nutrition/Prep.vue` | Onglet « Préparer » en trois étapes : plats et portions, courses par rayon, conseils de préparation. |
| `components/nutrition/Micros.vue` | Couverture en micronutriments et compléments justifiés. |
| `components/nutrition/Photo.vue` | Bouton photo d'un plat : prise de vue, vignette, aperçu plein format, suppression. |
| `lib/photoSize.ts` | Règles de dimensionnement et budget de stockage. Pur, testé. |
| `composables/usePhotos.ts` | Stockage IndexedDB des photos, redimensionnement canvas, cache d'URL d'objet. |
| `test/unit/photoSize.test.ts` | 17 tests : redimensionnement, refus de format, verdict de stockage. |
| `public/plats-demo/*.webp` | 16 illustrations de repli, une par plat livré. Générées, floues, ~2 Ko pièce. |
| `components/sport/Body.vue` | Suivi du corps, en tête du Rapport : pesées, courbe lissée, pente hebdomadaire, décomposition gras/muscle, IMC, pas, saisie manuelle. |
| `lib/withings.ts` | Décodage des mesures Withings et statistiques corporelles. Pur, testé, sans réseau. |
| `composables/useWithings.ts` | Jetons, synchronisation, persistance des pesées et des pas. |
| `server/api/withings/_client.ts` | Appels signés vers Withings. **Seul endroit qui voit le `client_secret`.** |
| `server/api/withings/authorize.get.ts` | Départ du flux OAuth2 + cookie `state` anti-CSRF. |
| `server/api/withings/callback.get.ts` | Échange du code contre les jetons, retour vers `/sport`. |
| `server/api/withings/sync.post.ts` | `getmeas` + `getactivity`, avec rafraîchissement de jeton transparent. |
| `test/unit/withings.test.ts` | 26 tests : décodage, fusion, lissage, pente, décomposition. |
| `assets/css/nutrition.css` | Styles, imbriqués sous `.sport-app` comme `sport.css`. Toutes les classes sont préfixées `nu-`. |
| `test/unit/nutritionStats.test.ts` | 98 tests de logique pure, dont le modèle de dépense et l’ajustement du dîner. |
| `test/nuxt/nutritionData.test.ts` | Tests de câblage localStorage, export/import, séance annulée. |

## Fichiers modifiés

- **`pages/sport.vue`** — import du CSS, `'nutrition'` ajouté au type `View`, une entrée
  dans `titles` et dans `TABS`, un bloc `<Suspense>` avec `<LazyNutritionPanel>`.
- **`components/sport/Profile.vue`** — la carte de pesée est partie dans Rapport, la
  semaine type (salle / télétravail) est arrivée à sa place : Profil ne porte plus que
  des réglages. La nutrition et les pesées partent dans la sauvegarde existante :
  `exportJSON({ profile, weekPlan, nutrition: nutritionData(), ...withingsData() })`.
  Les anciens fichiers passent toujours (les blocs ajoutés sont optionnels).
- **`components/sport/History.vue`** — réduit au calendrier ; le détail d'un jour est
  dans `DaySheet.vue`.
- **`components/sport/Report.vue`** — la carte « Corps » (poids, IMC, maintien) est
  remplacée par `<SportBody>`. L'IMC n'est plus calculé qu'à un seul endroit.
- **`composables/useWorkout.ts`** — `setBodyWeightAt(date, kg)` : le miroir du suivi
  corporel a besoin d'écrire à une date arbitraire, pas seulement aujourd'hui.

- **`nuxt.config.ts`** — bloc `runtimeConfig.withings` (`clientId`, `clientSecret`),
  volontairement **hors de `public`**.

Rien d'autre à toucher : pas de `routeRules` à ajouter (on reste sur `/sport`, déjà en
`ssr: false`), pas de store Pinia, pas de dépendance nouvelle.
...
2. URL de rappel : `https://gregoire-raturat.fr/api/withings/callback` — et, pour le
...
`/sport`. Un jeton volé donne accès aux pesées ; un secret volé donne accès à
...
(`/sport?withings=ok&access_token=…`). `adoptWithings()` les range immédiatement puis
...
`pages/sport.vue` qui fournit son contenu, le bandeau ne connaît rien des séances.
...
vérité unique pour les calculs (le même qui a produit le plan). Si tu veux changer une
...
Les tests unit passent (238/238 : 136 nutrition + 46 séances + 39 Withings + 17 photos). Ceux du projet `nuxt` n'ont pas pu être exécutés ici
(pas de `node_modules` installés dans l'environnement de développement du module) —
lance-les avant de committer.
