...
describe('planning hebdo', () => {
  it('remet le planning par défaut une seule fois puis respecte les modifications', async () => {
    // Ancien planning « adaptatif » qui avait dérivé : jeudi ET vendredi en s4,
    // plus de jour Jambes — exactement le bug constaté.
    localStorage.setItem('gr-weekplan-v1', JSON.stringify(['s1', 's2', null, 's4', 's4', null, null]))
    ...
    expect(weekPlan.value[3]).toBe('s3') // jeudi = Jambes rétabli
    expect(localStorage.getItem('gr-plan-fixed-v1')).toBe('1')
  })

  it('ne rejoue pas le nettoyage quand il a déjà eu lieu', async () => {
    localStorage.setItem('gr-plan-fixed-v1', '1')
