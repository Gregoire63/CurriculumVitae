  ...
  >
    <template #default>
      ...
      <template v-if="freePresets.length">
        ...
        <div class="pk-list">
          <button v-for="m in freePresets" :key="m.label" class="pk-opt fm-opt" @click="prefill(m)">
            <span class="pk-name">{{ m.label }}</span>
            <span class="pk-left mono">{{ m.kcal }} kcal · {{ m.p }} g P</span>
            <button class="nu-del" aria-label="Oublier" @click.stop="removeFreePreset(m.label)">×</button>
          </button>
        </div>
      </template>
